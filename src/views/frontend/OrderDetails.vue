    ...
    <div v-if="Object.keys(this.orderDetails).length !== 0">
      <ul class="flex justify-between w-full mx-auto mb-16 sm:w-7/12 md:w-1/2 xl:w-2/5">
        <li class="flex justify-around w-1/4">
          <p
            class="inline-block"
            :class="orderDetails.user.order.status === '收到訂單' ? ' text-brown-300 font-bold' : ' text-gray-300'"
            >收到訂單</p
          >
          <span
        ...
        <li class="flex justify-around w-1/4">
          <p
            class="inline-block"
            :class="orderDetails.user.order.status === '準備中' ? ' text-brown-300 font-bold' : ' text-gray-300'"
            >準備中</p
          >
          <span
            class="material-symbols-outlined"
            ...
            <tbody>
              ...
              <tr class="border-b-2 border-dashed border-brown-100">
                ...
                <td
                  ...
                  >{{ orderDetails.user.order.is_paid ? '已付款' : '尚未付款' }}</td
                >
              </tr>
              <tr class="border-b-2 border-dashed border-brown-100">
    ...
    getOrderDetails() {
      ...
      this.$http.get(api).then((res) => {
        if (res.data.order === null) {
          ...
          this.orderDetails.user.email =
            this.orderDetails.user.email.slice(0, 3) +
            '*'.repeat(index - 3) +
            this.orderDetails.user.email.slice(index);

          this.orderDetails.user.tel =
            this.orderDetails.user.tel.slice(0, 2) +
