<template>
  ...
  <main class="mt-[126px] min-h-[calc(100vh_-_330px)]">
    <div class="container font-medium text-black-light">
      <div class="flex flex-col justify-center lg:gap-20 lg:flex-row">
        <article class="lg:mb-16 lg:w-8/12 xl:w-6/12">
          ...
          <h2 class="mb-6 text-3xl font-semibold text-black font-noto-serif">{{ articleDetails.title }}</h2>

          <img :src="articleDetails.image" :alt="articleDetails.title" class="sm:float-right sm:w-[300px] sm:ml-6 mb-6" />

          <div
            v-html="articleDetails.content"
            class="min-h-[330px] mb-2 tracking-wide border-b-2 border-dashed lg:mb-7 text-black-light pb-14 lg:pb-7 border-brown-100"
          ></div>
        ...
        <aside class="lg:w-4/12 xl:w-3/12">
          <h3
    ...
    getArticle() {
      ...
      this.$http.get(api).then((response) => {
        if (response.data.success) {
          ...
          const api2 = `${import.meta.env.VITE_APP_API}/api/${import.meta.env.VITE_APP_PATH}/articles?page=1`;

          this.$http.get(api2).then((res) => {
            if (res.data.success) {
              this.articleList = res.data.articles;

              const matchId = this.articleList.find((article) => article.id === this.articleDetails.id);
              if (matchId) {
                const index = this.articleList.indexOf(matchId);
