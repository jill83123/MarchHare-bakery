<template>
  <nav
    ...
    data-te-navbar-ref
  >
    <div class="container flex flex-wrap items-center justify-between w-full px-3 text-white">
      <a href="/" class="flex items-center py-3">
        ...
        <h1 class="ml-2 text-2xl font-medium font-maru">三月兔<span class="ml-2 text-sm">下午茶</span></h1>
      </a>
      ...
      <div class="flex items-center">
        ...
        <button
          class="px-2 ml-4 text-xl leading-none transition-shadow duration-150 ease-in-out bg-transparent border-0 lg:hidden"
          type="button"
      ...
      >
        ...
        <!-- cart list -->
        <div
          class="!visible hidden max-h-[500px] min-w-[375px] mt-3 absolute right-0 top-[100%] bg-[rgb(255,255,255,0.95)] rounded-md border"
          data-te-collapse-item
          id="collapseWithScrollbar"
        >
          <div class="p-4 pr-2 text-black-light">
            <h3 class="pb-3 mb-3 border-b-2 border-gray-300 border-dotted">購物車列表</h3>
            <ul
              class="flex flex-col pb-3 mb-3 cart-list max-h-[338px] overflow-y-auto"
              v-if="cartList && cartList.length >= 1"
            >
              <li class="flex gap-2 py-3" v-for="cartItem in cartList" :key="cartItem.id">
                <div class="w-1/4 aspect-square grow">
                ...
                <div class="flex flex-col justify-between w-3/4 px-2 grow">
                  ...
                  <div class="flex items-center">
                    ...
                    >
                      <span class="text-xl text-gray-600 align-bottom material-symbols-outlined"
                        >do_not_disturb_on</span
                      >
                    </button>
                    <input
                      type="text"
                      class="outline-none focus:outline-none max-w-[30px] bg-transparent text-center"
                      v-model.lazy="cartItem.qty"
                      @change="updateCart(cartItem.id, cartItem.qty === 0 ? 1 : cartItem.qty)"
                    />
                    <span class="pl-1 text-sm text-gray-400">{{ cartItem.product.unit }}</span>
                    <button @click.prevent="updateCart(cartItem.id, cartItem.qty + 1)">
                      <span class="pl-1 text-xl text-gray-600 align-bottom material-symbols-outlined">add_circle</span>
                    </button>
                  ...
                  <div class="flex justify-between">
                    ...
                    <button class="self-end hover:text-danger" @click.prevent="delCartItem(cartItem.id)">
            ...
            <p class="text-center" v-if="!cartList"
              >目前購物車是空的<br />快來
              <RouterLink
                to="/shop"
                type="button"
                class="z-10 items-center inline-block px-1 ml-auto font-medium leading-normal tracking-wider uppercase transition duration-150 ease-in-out border-b text-brown-300 border-brown-300 hover:opacity-80"
                >點我</RouterLink
              >
              進行選購吧！</p
            >
            <div class="flex items-center pt-4 mt-3 border-t-2 border-gray-300 border-dotted">
              <p>共 NT {{ $filters.currency(cartTotalPrice) }} 元</p>
              <button
                type="button"
                class="z-10 flex items-center px-8 py-2 ml-auto text-sm font-medium leading-normal tracking-wider text-white uppercase transition duration-150 ease-in-out rounded-full bg-brown-300 focus:outline-none focus:ring-0 active:bg-cerulean-700 hover:opacity-80"
                >前往結帳
              </button>
            </div>
          </div>
...
export default {
  computed: {
    ...mapState(cartStore, ['cartList', 'cartTotalPrice']),
  },
  methods: {
    stickyNav() {
      const { nav } = this.$refs;
